...
// Usage: rt_pipeline_smoke <raygen.bin> <miss.bin> <closesthit.bin>
#include <bgfx/bgfx.h>
#include <cstdio>
#include <cstdint>
#include <vector>
...
{
	if (argc < 4) { printf("usage: %s <raygen.bin> <miss.bin> <closesthit.bin>\n", argv[0]); return 1; }
	...
	bgfx::ShaderHandle mi = bgfx::createShader(loadBin(argv[2]) );
	bgfx::ShaderHandle ch = bgfx::createShader(loadBin(argv[3]) );
	printf("shaders valid: rg=%d miss=%d chit=%d\n", bgfx::isValid(rg), bgfx::isValid(mi), bgfx::isValid(ch) );
	if (!bgfx::isValid(rg) || !bgfx::isValid(mi) || !bgfx::isValid(ch) ) { bgfx::shutdown(); return 1; }

	bgfx::ProgramHandle prog = bgfx::createRtProgram(rg, mi, ch, true);
	...
	printf(pass ? "RESULT: PASS (ray-tracing pipeline hit)\n" : "RESULT: FAIL (no hit)\n");
