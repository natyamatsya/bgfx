...
int main(int argc,char**argv){
	if(argc<7){printf("usage: %s <rq.bin> <atrous.bin> <rot.ppm> <fresh.ppm> <still.ppm> <temporal.bin>\n",argv[0]);return 1;}
	const uint32_t kFrames = 1;
	bgfx::renderFrame();
	bgfx::Init init; init.type=bgfx::RendererType::Count; init.resolution.width=0; init.resolution.height=0;
	if(!bgfx::init(init)){printf("init fail\n");return 1;}
	if(0==(bgfx::getCaps()->supported&BGFX_CAPS_RAY_TRACING)){printf("no RT\n");bgfx::shutdown();return 2;}
	...
	g_prog=bgfx::createProgram(bgfx::createShader(loadBin(argv[1])),true);
	g_atrous=bgfx::createProgram(bgfx::createShader(loadBin(argv[2])),true);
	if(!bgfx::isValid(g_prog)||!bgfx::isValid(g_atrous)){printf("prog fail\n");bgfx::shutdown();return 1;}
	g_params=bgfx::createUniform("u_params",bgfx::UniformType::Vec4);
	g_fparams=bgfx::createUniform("u_filterParams",bgfx::UniformType::Vec4);
	g_tp=bgfx::createUniform("u_tparams",bgfx::UniformType::Vec4);
	g_pt=bgfx::createUniform("u_ptParams",bgfx::UniformType::Vec4);
	g_temporal=bgfx::createProgram(bgfx::createShader(loadBin(argv[6])),true);
	...
	g_irr1  =bgfx::createTexture2D(kW,kH,false,1,bgfx::TextureFormat::RGBA32F,BGFX_TEXTURE_COMPUTE_WRITE);
	for (int i=0;i<2;i++){
	g_histI[i]=bgfx::createTexture2D(kW,kH,false,1,bgfx::TextureFormat::RGBA32F,BGFX_TEXTURE_COMPUTE_WRITE);
	g_histN[i]=bgfx::createTexture2D(kW,kH,false,1,bgfx::TextureFormat::RGBA32F,BGFX_TEXTURE_COMPUTE_WRITE);
	g_histM[i]=bgfx::createTexture2D(kW,kH,false,1,bgfx::TextureFormat::RGBA32F,BGFX_TEXTURE_COMPUTE_WRITE);
	g_res[i]=bgfx::createTexture2D(kW,kH,false,1,bgfx::TextureFormat::RGBA32F,BGFX_TEXTURE_COMPUTE_WRITE);}
	...
	BX_UNUSED(kFrames);
	// Rotating at 1 spp, 60 frames of temporal accumulation while the boxes move, then
	// read back mid-motion. With reprojection this should be clean and unghosted.
	float ang = 0.0f;
	// Single fresh frame, stage 3 (PT + denoiser) vs stage 4 (ReSTIR + denoiser).
	g_resetHist = true;
	dispatchFrame(0.0f, true, 2);
	bgfx::frame();
	readbackPPM(argv[3]);
	g_resetHist = true;
	dispatchFrame(0.0f, true, 3);
	bgfx::frame();
	readbackPPM(argv[4]);
	// ReSTIR converging with reuse: 8 more frames, read the 8th.
	for (int f = 0; f < 8; ++f) { dispatchFrame(0.0f, false, 3); bgfx::frame(); }
	readbackPPM(argv[5]);
	if (true) { bgfx::shutdown(); return 0; }
	for (int f = 0; f < 600; ++f)  // 7.2 rad: past a full rotation, where the smear lived
	{
		ang += 0.012f;
		updateXf(g_tlas, ang);
		dispatchFrame(ang, false, 2);
		bgfx::frame();
	}
	readbackPPM(argv[4]);
	// And one single fresh frame WITHOUT history (reset) at the same pose for contrast.
	// Static convergence at an axis-aligned pose: rotate back to 2*pi (= angle 0 pose).
	g_resetHist = true;
	for (int f = 0; f < 60; ++f) { dispatchFrame(6.2831853f, false, 2); bgfx::frame(); }
	readbackPPM(argv[5]);
