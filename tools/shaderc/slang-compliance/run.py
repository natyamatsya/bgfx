...
def parse_envelope(path):
    ...
    # writeUniformArray*, so every backend's envelope carries it.
    if r["ver"] >= 12:
        r["rawSrv"] = u32(); r["rawUav"] = u32()
    count = u16()
    r["uniforms"] = []
...
def compile_stock(shaderc, root, ex, stock_base, stage, out):
    ...
    cmd = [shaderc, "-f", sc, "--type", stage, "-p", "spirv", "--platform", "linux",
...
def compile_slang(shaderc, root, slang, stage, out):
...
def compare(ref, got):
    ...
    if _norm_matrix(ref["matrix"]) != _norm_matrix(got["matrix"]):
...
def main():
    ...
    for ex, stock_base, stage, slang in cases:
        ...
        if not os.path.exists(sc):
            continue
        label = f"{ex}/{stock_base} [{stage}]"
        if args.filter and args.filter not in label:
            nskip += 1
            continue
        ref_bin = os.path.join(tmp, "ref.bin")
        got_bin = os.path.join(tmp, "got.bin")
        rc1, log1 = compile_stock(shaderc, root, ex, stock_base, stage, ref_bin)
        rc2, log2 = compile_slang(shaderc, root, slang, stage, got_bin)
        if rc1 != 0:
        ...
        diffs = compare(parse_envelope(ref_bin), parse_envelope(got_bin))
        if diffs:
            print(f"FAIL {label}")
            for d in diffs:
                print(f"       {d}")
