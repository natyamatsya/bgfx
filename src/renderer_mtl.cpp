...
#	endif // __MAC_OS_X_VERSION_MAX_ALLOWED >= 101300
		...
		uint64_t encodeRtGlobalsSlot(const Binding& _bind)
		{
			...
			case Binding::Image:
				...
				return m_textures[_bind.m_idx].getTextureMipLevel(_bind.m_firstMip)->gpuResourceID()._impl;
			default:
				return 0;
			}
		}
		...
		void submitRayTracingDispatch(ProgramMtl& _program, const RenderCompute& _compute, const RenderBind& _renderBind, uint64_t _uniformsGpuAddr)
		{
			for (uint8_t ii = 0; ii < _program.m_numRtKernelArgs; ++ii)
			{
				...
				{
				case Binding::Image:
					m_computeCommandEncoder->setTexture(m_textures[bind.m_idx].getTextureMipLevel(bind.m_firstMip), ka.m_index);
					break;
				case Binding::IndexBuffer:
				case Binding::VertexBuffer:
					{
				...
				for (uint8_t ti = 0; ti < owner->m_rtGlobalsTailCount; ++ti)
				{
					...
					if (kInvalidHandle != bind.m_idx)
					{
						...
						case Binding::AccelerationStructure:
							...
							break;
						case Binding::Image:
							value = m_textures[bind.m_idx].getTextureMipLevel(bind.m_firstMip)->gpuResourceID()._impl;
